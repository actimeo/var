...
CREATE OR REPLACE FUNCTION mainview_set(prm_token integer, prm_mme_id integer, prm_title text, prm_icon text, prm_pme_id_associated integer)
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
  ...
  IF NOT FOUND THEN
    INSERT INTO portal.mainview (mme_id, mvi_title, mvi_icon, pme_id_associated) 
      VALUES(prm_mme_id, prm_title, prm_icon, prm_pme_id_associated);
  END IF;
END;
$$;
COMMENT ON FUNCTION mainview_set(prm_token integer, prm_mme_id integer, prm_title text, prm_icon text, prm_pme_id_associated integer) IS 'Set or update information about a view attached to a specified main menu';
